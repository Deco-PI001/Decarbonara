...
def graf_erzeugen(p_server_name, p_etage, p_raum, p_sensorklasse, p_name_des_wertes, p_datum_von, p_datum_bis):
    ...
    try:
        ...
        sql_time_max = time.max.__str__()
        sql_datum_bis =  p_datum_bis + ' ' + sql_time_max
...
arbeits_pfad = os.getenv('HOME') + '/Alles/Kunden/Decarbonara_GIT/Decarbonara/Python'
...
cb_zeitreaum_von['values'] = a_zeitraum #.strftime('%d%m%Y') 
